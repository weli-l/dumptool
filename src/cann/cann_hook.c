...
static void get_log_filename(time_t current, uint32_t pid, char *buf, size_t buf_size) {
...
static char is_ready_to_write(ThreadData *td, time_t *current) {
    ...
    *current = time(NULL);
    if (proc_mem->n_mem_alloc_stacks + proc_mem->n_mem_free_stacks < LOG_ITEMS_MIN) {
...
static void write_protobuf_to_file() {
    ...
    if (pthread_mutex_trylock(&file_mutex) == 0) { // pthread_mutex_trylock or pthread_mutex_lock
        char filename[256];
        get_log_filename(current, td->proc_mem->pid, filename, sizeof(filename));
