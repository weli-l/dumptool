...
// explicit：构造函数被标记为
// explicit，以防止隐式类型转换。这确保了只有在显式调用构造函数时才会创建
// ScopeGuard 对象。 std::function<void()> cb：构造函数接受一个 std::function
// 对象作为参数，该对象封装了一个可调用的回调函数。这个回调函数将在 ScopeGuard
// 对象被销毁时执行。 cb_(cb)：将传入的回调函数存储到成员变量 cb_ 中。
class ScopeGuard
{
  public:
    explicit ScopeGuard(std::function<void()> cb)
        : cb_(cb)
    {} // 当 ScopeGuard 对象被销毁时（即离开其作用域时），析构函数会被调用。
  ...
  public:
    explicit ShmType(int local_world_size, int local_rank, bool main = true)
    {
        ...
        if (main)
        {
            ...
            shm_area_ = new bip::managed_shared_memory(
                bip::create_only, shm_name_.c_str(), total_size);
            obj_ = shm_area_->construct<T>(obj_name.c_str())();
            obj_->reset();
            LOG(INFO) << "ShmType rank " << local_rank
                      << " create shared memory object " << shm_name_
                      << std::endl;
            InterProcessBarrier(local_world_size, local_rank,
                                barrier_name.c_str());
        }
        else
        {
            LOG(INFO) << "ShmType rank " << local_rank
                      << " open shared memory object " << shm_name_
                      << std::endl;
            InterProcessBarrier(local_world_size, local_rank,
                                barrier_name.c_str());
            shm_area_ = new bip::managed_shared_memory(bip::open_only,
                                                       shm_name_.c_str());
  ...
  public:
    ...
    static T GetEnvVar(const std::string &name)
    {
        ...
        if (auto it = registry.find(name); it != registry.end())
        {
            ...
            //   if constexpr (Print)
            //     LOG(INFO) << "[ENV] Get " << name << "=" << result << " from
            //     config" << std::endl;
            //   return result;
            // }
            ...
            {
                // GetEnvVar is a internal api, so you should verify it, it not,
                // we abort
                if constexpr (Print)
                    LOG(FATAL)
